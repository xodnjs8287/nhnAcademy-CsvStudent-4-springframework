...
public class DefaultStudentService implements StudentService {
    ...
    public Collection<Student> getPassedStudents() {
        ...
        return students.stream()
                .filter(it -> Objects.nonNull(it.getScore()))
                .filter(it -> !it.getScore().isFail())
    ...
    public Collection<Student> getStudentsOrderByScore() {
        ...
        Comparator<Student> comparing = Comparator.comparing(it -> it.getScore().getScore());
        Comparator<Student> reversed = comparing.reversed();

        return studentRepository.findAll().stream().sorted(reversed).collect(Collectors.toList());
    }
